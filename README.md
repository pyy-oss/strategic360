...
- **IA (`classifyAI`/`syncSources`/`generateBriefing`) — Vertex AI, endpoint `global`** : les
  endpoints régionaux (`us-central1`, `europe-west1`) ont tous deux renvoyé 404 sur chaque modèle
  Gemini essayé contre ce projet ("Publisher model ... was not found"), IAM et API pourtant
  corrects (Vertex AI Studio fonctionne sur ce même projet). Les générations récentes de Gemini
  sur Vertex AI sont servies via l'**endpoint global** — `VERTEX_LOCATION=global` (défaut du code
  et de `functions/.env.propulse-business-87f7a`), modèle `gemini-3.5-flash` (celui du sélecteur
  de Studio). Authentification par compte de service (ADC), rien à provisionner. Voir le
  DIAGNOSIS HISTORY dans `functions/domain/vertex.js` pour l'enquête complète ; si `global` 404
  aussi, le plan B validé (API Gemini Developer, clé API en Secret Manager) est dans l'historique
  git (commit "Switch Vertex AI calls to Gemini Developer API").
